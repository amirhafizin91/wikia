...
[Prev](sect_08_01.html)
Chapter 8. Writing interactive scripts
[Next](sect_08_03.html)
...
[]()8.2. Catching user input {#catching-user-input .sect1}
============================

<div class="sect2">

[]()8.2.1. Using the read built-in command {#using-the-read-built-in-command .sect2}
------------------------------------------
...
3.4.8](sect_03_04.html#sect_03_04_07). The backslash character may be
...
|                                                                          |
| echo "Type the year that you want to check (4 digits), followed by [ENTE |
| R]:"                                                                     |
|                                                                          |
| read year                                                                |
|                                                                          |
...
[]()8.2.2. Prompting for user input {#prompting-for-user-input .sect2}
-----------------------------------
...
| Hello, michel.  This script will register you in Michel's friends databa |
| se.                                                                      |
...
SGID](http://www.tldp.org/LDP/intro-linux/html/sect_04_01.html#sect_04_01_06)
...
[]()8.2.3. Redirection and file descriptors {#redirection-and-file-descriptors .sect2}
-------------------------------------------

<div class="sect3">

### []()8.2.3.1. General {#general .sect3}
...
| ``` {.screen}                                                            |
| michel ~> ls -l /dev/std*                                               |
| lrwxrwxrwx  1 root    root     17 Oct  2 07:46 /dev/stderr -> ../proc/se |
| lf/fd/2                                                                  |
| lrwxrwxrwx  1 root    root     17 Oct  2 07:46 /dev/stdin -> ../proc/sel |
| f/fd/0                                                                   |
| lrwxrwxrwx  1 root    root     17 Oct  2 07:46 /dev/stdout -> ../proc/se |
| lf/fd/1                                                                  |
|                                                                          |
| michel ~> ls -l /proc/self/fd/[0-2]                                      |
| lrwx------  1 michel  michel   64 Jan 23 12:11 /proc/self/fd/0 -> /dev/p |
| ts/6                                                                     |
| lrwx------  1 michel  michel   64 Jan 23 12:11 /proc/self/fd/1 -> /dev/p |
| ts/6                                                                     |
| lrwx------  1 michel  michel   64 Jan 23 12:11 /proc/self/fd/2 -> /dev/p |
| ts/6                                                                     |
| ```                                                                      |
+--------------------------------------------------------------------------+
...
-   Redirection <span class="QUOTE">"N&gt;&M"</span> or <span
    class="QUOTE">"N&lt;&M"</span> after a command has the effect of
    creating or updating the symbolic link `/proc/self/fd/N`{.filename}
    with the same target as the symbolic link
    `/proc/self/fd/M`{.filename}.

-   The redirections <span class="QUOTE">"N&gt; file"</span> and <span
    class="QUOTE">"N&lt; file"</span> have the effect of creating or
    updating the symbolic link `/proc/self/fd/N`{.filename} with the
    target file.

-   File descriptor closure <span class="QUOTE">"N&gt;&-"</span> has the
    effect of deleting the symbolic link `/proc/self/fd/N`{.filename}.
...
| total 0                                                                  |
| lr-x------    1 michel michel  64 Jan 24 11:07 0 -> /var/spool/at/!0000c |
| 010959eb (deleted)                                                       |
...
### []()8.2.3.2. Redirection of errors {#redirection-of-errors .sect3}

From the previous examples, it is clear that you can provide input and
output files for a script (see [Section
8.2.4](sect_08_02.html#sect_08_02_04) for more), but some tend to forget
...
This is the equivalent of **&gt; `FILE`{.filename} 2&gt;&1**, the
construct used in the previous set of examples. It is also often
combined with redirection to `/dev/null`{.filename}, for instance when
you just want a command to execute, no matter what output or errors it
gives.
...
[]()8.2.4. File input and output {#file-input-and-output .sect2}
--------------------------------

<div class="sect3">

### []()8.2.4.1. Using /dev/fd {#using-devfd .sect3}
...
| This text is printed at the beginning of each print job and thanks the s |
| ysadmin                                                                  |
...
### []()8.2.4.2. Read and exec {#read-and-exec .sect3}

<div class="sect4">

#### []()8.2.4.2.1. Assigning file descriptors to files {#assigning-file-descriptors-to-files .sect4}
...
+--------------------------------------+--------------------------------------+
| ![Note](../images/note.gif)          | **File descriptor 5**                |
+--------------------------------------+--------------------------------------+
|                                      | Using this file descriptor might     |
|                                      | cause problems, see [the Advanced    |
|                                      | Bash-Scripting                       |
|                                      | Guide](http://www.tldp.org/LDP/abs/h |
|                                      | tml/io-redirection.html),            |
|                                      | chapter 16. You are strongly advised |
|                                      | not to use it.                       |
+--------------------------------------+--------------------------------------+
...
#### []()8.2.4.2.2. Read in scripts {#read-in-scripts .sect4}
...
| #!/bin/bash                                                              |
|                                                                          |
| # This script makes an index of important config files, puts them togeth |
| er in                                                                    |
...
### []()8.2.4.3. Closing file descriptors {#closing-file-descriptors .sect3}
...
### []()8.2.4.4. *Here* documents {#here-documents .sect3}
...
class="QUOTE">"Is this ok \[y/N\]"</span> string, the script answers
<span class="QUOTE">"y"</span> automatically:
...
------------------------------------------------------------------------
  ...
  -------------------------- -------------------- -------------------------
  [Prev](sect_08_01.html)    [Home](index.html)   [Next](sect_08_03.html)
  Displaying user messages   [Up](chap_08.html)   Summary
  -------------------------- -------------------- -------------------------
