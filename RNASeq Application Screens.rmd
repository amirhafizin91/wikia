...
![alt text](https://github.com/amirhafizin91/wikia/blob/Product-Development/1st.png)
...
SVN checkout the code from this location and stick the whole `webviews` folder under your webserver

## External Database Annotation Files
We source our external gene information, annotation files , etc, for model organisms from the http://genome.ucsc.genome.edu UCSC Genome database site. These files become important in resolving refseq identifies/accessions into gene names, gene descriptions, etc.

Annotation files of interest are available from  http://hgdownload.cse.ucsc.edu/goldenPath/mm9/database/

Note that !`mm9` is the database code name in UCSC, for the human genome it is hg18 (corresponding to NCBI36).

|**File**|**Description**|
|refFlat.txt.gz` | Refseq flat file database containing links to gene names. These are indexed by Refseq accessions e.g. NM_00032323 |
| refGene.txt.gz | Refseq file indexed by gene name ||
| refLink.txt.gz | Refseq gene product name descriptions e.g. protein,mRNA ids |

## Required Reference files
We need to  supply the framework RNASeq pipeline with:

* Gene coordinate files containing genome positions of exons, cds,etc (GTF format)
* Gene annotation file containing accessions/IDs in GTF files that map to real gene names and their descriptions
* Genome index to map short reads to with chosen aligner
